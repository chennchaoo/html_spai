...
$check = $_SGLOBAL['pdo']->fetOne("casio4_infos","*","cid = '$cid'");
if(!$check){
    msg(1006);
}
if($check['status'] != 1){
...
$TK = array();
//$vote = $_SGLOBAL['pdo']->fetOne("casio4_vote","*","member_id = '$xtgroup_uid' AND cid = '$cid'");
...
if(!$vote){
	...
	try{
            ...
            if(!$_SGLOBAL['pdo']->add('casio4_vote',$info)){
                msg(2003);
            }
            if(!$_SGLOBAL['pdo']->update('casio4_infos',"votes = votes + 1","cid = '$cid'")){
                msg(2003);
            }          
            //自己数据
            /*
            if(!$_SGLOBAL['pdo']->update('casio4_member',"hadnums = hadnums + 1","member_id = '$xtgroup_uid'")){
                msg(2003);
            }*/
            $_SGLOBAL['pdo']->commit();
	}catch (Exception $e){
