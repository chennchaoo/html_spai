...
var dropload = $('.inner').dropload({
	...
	$.ajax({
		...
		data:'page='+p+'&perpage=8',
		success : function(data){
			if(data.code==1){
				if(data.data.total-8>0){
					var nums='0';
					for(var i = 0; i < parseInt(data.data.total/2); i++){
			
					result +=	'<div class="voteList">'
								   + '<div class="list_da">'
									+    '<div class="list_da_left">'
									 +      ' <div class="list_da_shang">'
									  +          '<div class="list_img"><img src="'+data.data.lists[nums].avatar+'" data-preview-src="" data-preview-group="1"></div>'
									   +         '<div class="list_zi">'
										+            '<div class="list_zi_1">选手:'+data.data.lists[nums].nickname+'</div>'
										 +           '<div class="list_zi_1">票数：'+data.data.lists[nums].vote_count+'</div>'
										  +          '<div class="list_zi_1">排名：第'+data.data.lists[nums].pm+'名</div>'
										   +     '</div>'
											+'</div>'
											+'<div class="list_da_xia" onclick="dianzan('+data.data.lists[nums].tlid+')"><img src="img/myLog.png"></div>'
										+'</div>'
										+'<div class="list_da_left">'
										 +   '<div class="list_da_shang">'
										  +      '<div class="list_img"><img src="'+data.data.lists[++nums].avatar+'" data-preview-src="" data-preview-group="1"></div>'
					...
					setTimeout(function(){
							/*$('.lists').append(result);*/
							$('.jinxing_over').before(result);
							me.resetload();
					},1000);
				}else{
					var wu='';
					setTimeout(function(){
					$('.jinxing_over').before(wu);
					me.resetload();
					},1000);
				}
			}else{
...
<script>
	...
	var shareUrl='<?php echo $TK['share']['shareUrl'] ?>';
	var shareTitle='<?php echo $TK['share']['title']?>';
	var shareContent='<?php echo $TK['share']['des']?>';
	var shareImg='<?php echo $TK['share']['img']?>';
$.ajax(
{
	type : "GET",
	//async : false,
